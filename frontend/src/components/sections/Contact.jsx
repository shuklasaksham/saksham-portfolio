  ...
  return (
    <section className="px-6 md:px-10 lg:px-12 py-8 border-t" style={{ borderColor: 'rgba(245,165,36,0.24)' }}>
      <div className="mb-4 flex items-center gap-3 text-[13px]" style={{ color: 'var(--amber)', letterSpacing: '0.15em' }}>
        <span className="font-semibold">C-MAIL — CONTACT</span>
      ...
      <article className="rounded-sm p-5 md:p-6" style={{ border: '1px solid var(--amber)' }}>
        ...
        <p className="mt-2 text-[14px]" style={{ color: 'var(--amber)' }}>
          Complete a sentence or write your own.
        </p>

        <div className="mt-4 flex flex-wrap gap-2">
          {contactPrompts.map((p) => (
            <button key={p} onClick={() => setMsg((m) => (m ? m + ' ' + p : p))} className="text-[13px] px-3 py-1.5" style={{ border: '1px solid rgba(245,165,36,0.55)', color: 'var(--amber)', borderRadius: 2 }}>
              {p}
            </button>
          ))}
        </div>
        ...
        <form onSubmit={submit} className="mt-4">
          <div className="flex gap-2 px-3 py-3" style={{ border: '1px solid rgba(245,165,36,0.4)', borderRadius: 2 }}>
            <span className="pt-1" style={{ color: 'var(--amber)' }}>&gt;</span>
            <textarea value={msg} onChange={(e) => setMsg(e.target.value)} placeholder="Start typing or pick a sentence above…" rows={4} className="w-full bg-transparent outline-none resize-none text-[14px] placeholder:opacity-50" style={{ color: 'var(--amber-2)', caretColor: 'var(--amber)' }} />
          </div>
        ...
        <div className="text-[13px]" style={{ color: 'var(--amber)' }}>
          Or find me elsewhere: {socials.map((s, i) => (
